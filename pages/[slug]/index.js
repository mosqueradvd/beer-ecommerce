...
function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    const [id, ...brandParts] = slug.split("-");
    const brand = brandParts.join("-").replace(/\-/g, " ");
    setProduct(
      products.find(
        (product) =>
          product.id === Number(id) && product.brand.toLowerCase() === brand
      )
    );
  }, [router.isReady]);
  ...
  useEffect(() => {
    const fetchSkuData = async () => {
      if (product && product.skus.length) {
        const res = await fetch(`/api/stock-price/${product.skus[0].code}`);
        const data = await res.json();
        setSkuData(data);
      }
    };

    const intervalId = setInterval(fetchSkuData, 5000); // Fetch every 5 seconds
  ...
  return (
    <>
      ...
      <div className="2xl:container 2xl:mx-auto lg:py-16 lg:px-20 md:py-12 md:px-6 py-9 px-4 ">
        <div className="flex justify-center items-center lg:flex-row sm:flex-col sm:flex-col-reverse flex-col gap-8">
          {/* <!-- Description Div --> */}
          ...
          <div
            className=" w-full sm:w-96 md:w-8/12 lg:w-6/12 flex lg:flex-row flex-col  sm:gap-6 gap-4"
            width={200}
          >
          ...
          <div className="w-full sm:w-96 md:w-8/12 lg:w-full grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 lg:gap-28 sm:gap-x-6 sm:gap-y-12 gap-y-12 sm:mt-14 mt-10">
            <div>
              ...
              <p className="text-normal text-base leading-6 text-gray-600 mt-4">
                Crafted with unique hops and barley, this beer refreshes like a
                crisp breeze, making it ideal for all your drink occasions.
              </p>
            </div>
            <div>
              <svg
              ...
              <p className="text-normal text-base leading-6 text-gray-600 mt-4">
                Infused with a complex blend of malt, our beer offers an
                explosion of delightful flavors, guaranteed to invigorate your
                taste buds.
              </p>
            </div>
            <div>
              ...
              <p className="text-normal text-base leading-6 text-gray-600 mt-4">
                From casual get-togethers to grand celebrations, this versatile
                beer suits every occasion with its appealing taste and aroma
              </p>
            </div>
            <div>
              <svg
              ...
              <p className="text-normal text-base leading-6 text-gray-600 mt-4">
                Brewed with camaraderie in mind, our beer is the perfect
                conversation starter to share with friends, creating memorable
                moments.
              </p>
            </div>
          </div>
